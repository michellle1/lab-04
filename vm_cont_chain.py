...
"""EE 250L Lab 04 starter code. This is Pong Publisher."""
...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe("mearredo/ping")
    
    #callback and publish new message 
    client.message_callback_add("mearredo/ping", on_message_from_ping)
    time.sleep(4)
...
#Custom message callback. And then publish a new message to subscribers (i.e to ping)
def on_message_from_ping(client, userdata, message):
   numNum = int(message.payload.decode()) #message from ping --> convert to int
   print("Custom callback  - integer: "+ f"{numNum}")
   
   newNum = numNum +1 #publish message to pong 
   time.sleep(4)
   client.publish("mearredo/pong", newNum)
   print("Publishing integer:" + f"{newNum}")
...
if __name__ == '__main__':
	...
	#create a client object
   	...
   	wrote clients and servers all in separate *processes*. However, every 
    	...
    	reconnecting.""" # does client.loop_forever() still work for part 2? because multiple threads?
